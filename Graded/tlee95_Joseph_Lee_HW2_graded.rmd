...
```{remark class.source="tacomments"}
[Efficiency 1] In this case using matrix is more verbose and harder to read versus using cbind
```
...
```{remark class.source="tacomments"}
[Efficiency 3] Very slow compared to e.g.  dat[-order(dat$x)[1:100],]
```
...
```{remark class.source="tacomments"}
[Beauty 1] I would not recommend using ;. Merging multiple lines causes readability problems
```
...
```{remark class.source="tacomments"}
[Correctness 1] You generate new random numbers rather than leveraging alpha.
```
...
```{remark class.source="tacomments"}
[Correctness 1] Wrong number of rows returned
```

```{remark class.source="tacomments"}
[Beauty 1] Readability could be improved by adding line breaks
```
...
```{remark class.source="tacomments"}
[Beauty 3] You do not leverage data.table to its best functionality. You can remove ccso$ from the above
```
